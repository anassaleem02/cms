...
export class AdminServicesComponent implements OnInit {
  ...
  iconOptions = ['tool', 'settings', 'clipboard', 'award', 'activity', 'package', 'zap', 'shield', 'wifi'];

  constructor(private service: ServiceItemService, private fb: FormBuilder) {}
  ...
  openCreate(): void {
    this.editingId = null;
    this.form = this.fb.group({ title: ['', Validators.required], description: ['', Validators.required], icon: ['tool'], isActive: [true], displayOrder: [0] });
    this.showForm = true;
  }

  openEdit(item: ServiceItem): void {
    this.editingId = item.id;
    this.form = this.fb.group({ title: [item.title, Validators.required], description: [item.description, Validators.required], icon: [item.icon], isActive: [item.isActive], displayOrder: [item.displayOrder] });
    this.showForm = true;
  }
